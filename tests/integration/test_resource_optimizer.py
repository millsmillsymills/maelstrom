...
class TestResourceOptimizer:
    ...
    def test_automated_optimizations(self, resource_optimizer):
        ...
        container_metrics = {
            ...
            "unhealthy-container": {"status": "exited", "health_status": "unhealthy"},
        }
    ...
    def test_monitoring_cycle_execution(self, resource_optimizer):
        ...
        with patch.object(
            resource_optimizer, "collect_host_metrics", return_value=mock_host_metrics
        ):
            with patch.object(
                resource_optimizer,
                ...
                with patch.object(
                    resource_optimizer, "analyze_resource_usage", return_value=([], [])
                ):
                    with patch.object(resource_optimizer, "store_metrics"):
                        with patch.object(
                            resource_optimizer, "apply_optimizations", return_value=[]
                        ):
    ...
    def test_docker_socket_access(self):
        ...
        try:
            ...
            found_services = [
                ...
                if any(s in name.lower() for name in container_names)
            ]
            assert (
                len(found_services) > 0
            ), f"No monitoring services found in containers: {container_names}"
    ...
    def test_influxdb_connectivity(self):
        ...
        try:
            ...
            client = InfluxDBClient(
                host="localhost", port=8086, database="test_resource_monitoring"
            )
