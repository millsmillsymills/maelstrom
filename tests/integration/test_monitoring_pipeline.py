    ...
    def test_metrics_collection_pipeline(self, influxdb_client, prometheus_client):
        ...
        try:
            ...
            for point in data_points:
                ...
                assert time_diff < timedelta(
                    minutes=10
                ), f"Prometheus data not recent: {time_diff}"
    ...
    def test_alerting_pipeline(self, prometheus_client):
        ...
        try:
            ...
            found_rules = [
                ...
                if any(expected in rule for expected in expected_rules)
            ]

            assert (
                len(found_rules) > 0
            ), f"No critical alerting rules found. Available: {rule_names[:5]}"
...
class TestServiceIntegration:
    ...
    def test_prometheus_service_discovery(self, prometheus_client):
        ...
        try:
            ...
            expected_exporters = ["node-exporter", "cadvisor", "prometheus"]
            ...
            unhealthy_targets = [
                target for target in active_targets if target["health"] == "down"
            ]

            # At least 50% of targets should be healthy
            health_ratio = (
                len(healthy_targets) / len(active_targets) if active_targets else 0
            )
            assert (
                health_ratio >= 0.5
            ), f"Too many unhealthy targets: {len(unhealthy_targets)}/{len(active_targets)}"
    ...
    def test_logging_pipeline(self):
        ...
        try:
            ...
            query_response = requests.get(
                "http://localhost:3100/loki/api/v1/labels", timeout=5
            )
    ...
    def test_wazuh_elasticsearch_integration(self):
        ...
        try:
            ...
            assert health_data["status"] in [
                "green",
                "yellow",
            ], f"Elasticsearch cluster unhealthy: {health_data['status']}"

            # Check for Wazuh indices (if any exist)
            indices_response = requests.get(
                "http://localhost:9200/_cat/indices?format=json", timeout=5
            )
            if indices_response.status_code == 200:
                indices = indices_response.json()
                wazuh_indices = [
                    idx for idx in indices if "wazuh" in idx.get("index", "")
                ]
                ...
                if url:
                    try:
                        ...
                        assert response.status_code in [
                            ...
                            401,
                            403,
                        ], f"{service} not responding properly"
    ...
    def test_network_discovery_integration(self, influxdb_client):
        ...
        try:
            ...
            network_measurements = [
                ...
                if "network" in m.lower() or "discovery" in m.lower()
            ]
