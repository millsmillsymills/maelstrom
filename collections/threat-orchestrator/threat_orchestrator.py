...
class ThreatEvent:
    ...
    def _determine_severity(self) -> ThreatSeverity:
        ...
        # Critical indicators
        if any(keyword in signature for keyword in ["exploit", "backdoor", "trojan", "malware"]):
            return ThreatSeverity.CRITICAL
        
        # High severity indicators
        if confidence > 0.8 or any(keyword in signature for keyword in ["attack", "intrusion", "breach"]):
            return ThreatSeverity.HIGH
        
        # Medium severity
        if confidence > 0.5 or any(keyword in signature for keyword in ["suspicious", "anomaly", "scan"]):
    ...
    def _get_recommended_actions(self) -> List[Dict[str, Any]]:
        ...
        if self.severity in [ThreatSeverity.MEDIUM, ThreatSeverity.HIGH, ThreatSeverity.CRITICAL]:
            actions.append({
                "type": "investigate",
                "target": source_ip,
...
class ThreatOrchestrator:
    ...
    async def process_threat(self, event: ThreatEvent) -> Dict[str, Any]:
        ...
        logger.info(f"Processing {event.severity.value} threat from {event.source}: {event.id}")
    ...
    async def _log_to_loki(self, event: ThreatEvent):
        ...
        try:
                ...
                "streams": [
                    {
                        ...
                        "values": [
                            [str(int(datetime.now().timestamp() * 1000000000)), json.dumps({
                                "event_id": event.id,
                                "severity": event.severity.value,
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(f"{LOKI_URL}/loki/api/v1/push", json=log_entry)
    ...
    async def _send_slack_alert(self, event: ThreatEvent):
        ...
        try:
            ...
            actions_text = "\n".join([
                ...
                for action in event.recommended_actions[:3]  # Limit to first 3 actions
            ])
            ...
            message = {
                ...
                "attachments": [
                    {
                        "color": "danger" if event.severity == ThreatSeverity.CRITICAL else "warning",
                        "fields": [
                            {
                            ...
                            {
                                "title": "Description",
                                "value": event.normalized.get("description", "No description"),
                                "short": False
                            },
                            {
                                "title": "Recommended Actions",
                                "value": actions_text or "Manual investigation recommended",
                                "short": False
                            }
                        ]
                    }
            ...
            if event.recommended_actions and any(action.get("runbook") for action in event.recommended_actions):
                runbook = next((action["runbook"] for action in event.recommended_actions if action.get("runbook")), None)
                if runbook:
...
async def receive_suricata_event(event_data: Dict[str, Any], background_tasks: BackgroundTasks):
...
async def receive_zeek_event(event_data: Dict[str, Any], background_tasks: BackgroundTasks):
...
async def receive_wazuh_event(event_data: Dict[str, Any], background_tasks: BackgroundTasks):
