...
class IoTDeviceMonitor:
    ...
    async def discover_devices(self):
        ...
        logger.info(
            f"Found {len(self.devices)} IoT devices, {len(self.edge_nodes)} edge nodes"
        )
    ...
    async def scan_host(self, ip: str):
        ...
        try:
            ...
            if device_info:
                ...
                logger.info(
                    f"Discovered {device.device_type.value}: {device.hostname} at {ip}"
                )
    ...
    async def fingerprint_device(
        ...
        device_info["type"] = self.classify_device_type(
            open_ports, device_info["capabilities"]
        )
            ...
            async with asyncio.timeout(5):
                ...
                if server:
                    ...
                    if "nginx" in server.lower():
                        info["capabilities"] = info.get("capabilities", []) + [
                            "web_server"
                        ]
    ...
    async def get_rtsp_info(self, ip: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(ip, 554), timeout=3.0
            )
    ...
    async def get_mqtt_info(self, ip: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(ip, 1883), timeout=3.0
            )
    ...
    def classify_device_type(
        self, open_ports: List[int], capabilities: List[str]
    ) -> str:
    ...
    async def process_upnp_response(self, response: str, ip: str):
        ...
        try:
            ...
            if "location" in headers:
                detailed_info = await self.get_upnp_device_description(
                    headers["location"]
                )
                if detailed_info:
    ...
    async def get_upnp_device_description(
        ...
        try:
            ...
            logger.debug(
                f"Failed to get UPnP device description from {location_url}: {e}"
            )
            return None
        ...
        try:
            ...
            for service_type in service_types:
                try:
                    ...
                    logger.debug(
                        f"mDNS discovery for {service_type} failed or timed out"
                    )
                    continue
            ...
            for ip in list(network.hosts())[:10]:  # Sample first 10 IPs
                for port in mqtt_ports:
                    try:
                        ...
                        reader, writer = await asyncio.wait_for(
                            asyncio.open_connection(str(ip), port), timeout=2.0
                        )
    ...
    async def analyze_discovered_devices(self):
        ...
        logger.info(
            f"Analysis complete: {vulnerability_count} vulnerabilities, {performance_issues} performance issues"
        )
    ...
    async def analyze_device_security(self, device: IoTDevice) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "mqtt_communication" in device.capabilities
                and 1883 in await self.port_scan(device.ip_address, [1883])
            ):
            ...
            if await self.check_weak_protocols(device.ip_address):
                ...
                security_info["recommendations"].append(
                    "Disable weak protocols (Telnet, HTTP)"
                )
    ...
    async def analyze_device_performance(self, device: IoTDevice) -> Dict[str, float]:
        ...
        try:
            ...
            metrics["estimated_bandwidth"] = (
                1.0  # Would implement actual bandwidth measurement
            )
    ...
    async def is_edge_node(self, ip: str, device_info: Dict[str, Any]) -> bool:
        ...
        edge_indicators = [
            ...
            device_info.get("manufacturer", "").lower()
            in ["raspberry", "nvidia", "intel"],
        ]
        ...
        while self.running:
            try:
                for ip, device in self.devices.items():
                    ...
                    device.status = (
                        DeviceStatus.ONLINE
                        if await self.is_host_alive(ip)
            ...
            if webhook_url:
                ...
                message = {
                    ...
                    "attachments": [
                        {
                            "color": (
                                "good" if online_devices == total_devices else "warning"
                            ),
                            "fields": [
                                {
                                ...
                                {
                                    "title": "Edge Nodes",
                                    "value": str(total_edge_nodes),
                                    "short": True,
                                },
                                {
                                    "title": "Vulnerabilities",
                                    "value": str(self.stats["vulnerabilities_found"]),
                                    "short": True,
                                },
                                {
                                    "title": "Performance Issues",
                                    "value": str(self.stats["performance_issues"]),
                                    "short": True,
                                },
                            ],
                        }
        ...
        return {
            ...
            "device_types": {
                device_type.value: sum(
                    1 for d in self.devices.values() if d.device_type == device_type
                )
                for device_type in DeviceType
            },
            "manufacturers": {
                manufacturer: sum(
                    1 for d in self.devices.values() if d.manufacturer == manufacturer
                )
