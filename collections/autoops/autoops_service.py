...
class ActionProposer:
                ...
                "streams": [
                    {
                        ...
                        "values": [
                            [str(int(datetime.now().timestamp() * 1000000000)), json.dumps(proposal)]
                        ]
                    }
                ]
            }
    ...
    async def notify_slack(self, proposal: Dict[str, Any]):
        ...
        try:
            ...
            confidence_emoji = "🟢" if proposal['confidence'] > 0.7 else "🟡" if proposal['confidence'] > 0.4 else "🔴"
            
            message = {
                "text": f"🤖 AutoOps Action Proposal",
                "attachments": [
                    {
                        ...
                        "fields": [
                            ...
                            },
                            {
                                ...
                                "value": "✅ Yes" if proposal['requires_approval'] else "❌ No",
                                "short": True
                            },
                            {
...
@app.get("/")
async def root():
    return {"service": "AutoOps", "status": "running", "approval_mode": APPROVAL_REQUIRED}

@app.post("/webhook/prometheus")
async def receive_prometheus_alert(alert_data: Dict[str, Any], background_tasks: BackgroundTasks):
