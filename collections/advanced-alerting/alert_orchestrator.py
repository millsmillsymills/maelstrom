...
class DynamicThresholdCalculator:
    ...
    def calculate_dynamic_threshold(
        ...
        recent_values = [
            value for timestamp, value in history if timestamp >= cutoff_time
        ]

        if len(recent_values) < 10:
            recent_values = [
                value for _, value in list(history)[-50:]
            ]  # Use last 50 points
        ...
        try:
            ...
            logger.debug(
                f"Dynamic threshold for {metric_name}: {threshold:.2f} (base: {base_threshold:.2f})"
            )
            return threshold
...
class AlertCorrelationEngine:
    ...
    def correlate_alerts(
        self, new_alert: Alert, active_alerts: List[Alert]
    ) -> Optional[str]:
        ...
        recent_alerts = [
            ...
            if abs((new_alert.first_seen - alert.first_seen).total_seconds())
    ...
    def correlate_by_service(
        self, new_alert: Alert, recent_alerts: List[Alert]
    ) -> Optional[str]:
    ...
    def correlate_by_metric(
        self, new_alert: Alert, recent_alerts: List[Alert]
    ) -> Optional[str]:
        """Correlate alerts by metric type"""
        metric_type = new_alert.source_metric.split("_")[
            0
        ]  # e.g., 'cpu' from 'cpu_usage'
...
This is an automated alert from the Maelstrom Monitoring System.""",
    ...
    def is_rate_limited(
        self, channel: NotificationChannel, alert_rule_id: str, rate_limit: int
    ) -> bool:
        ...
        while (
            ...
            and (now - self.rate_limiters[key][0]).total_seconds() > rate_limit
        ):
    ...
    async def send_notification(
        self, alert: Alert, rule: AlertRule, channels: List[NotificationChannel] = None
    ):
        ...
        for channel in channels:
            if self.is_rate_limited(channel, rule.rule_id, rule.suppress_duration):
                logger.info(
                    f"Notification rate limited for {channel.value}:{rule.rule_id}"
                )
                continue

            task = asyncio.create_task(
                self.send_channel_notification(alert, rule, channel)
            )
    ...
    def format_notification_message(
        self, alert: Alert, rule: AlertRule, template: NotificationTemplate
    ) -> Dict[str, Any]:
    ...
    async def send_slack_notification(
        self, message_data: Dict[str, Any], template: NotificationTemplate
    ):
    ...
    async def send_webhook_notification(
        self, message_data: Dict[str, Any], alert: Alert
    ):
...
class AlertOrchestrator:
    ...
    def add_alert_rule(self, rule: AlertRule):
        ...
        if rule.correlation_rules:
            self.correlation_engine.add_correlation_rule(
                rule.rule_id, {"related_rules": rule.correlation_rules}
            )
    ...
    async def evaluate_metric(
        ...
        try:
            ...
            self.threshold_calculator.update_metric_data(
            ...
            applicable_rules = [
                ...
                or metric_name.startswith(rule.metric_name)
            ]
    ...
    async def evaluate_rule(
        self, rule: AlertRule, value: float, metadata: Dict[str, Any]
    ):
        ...
        try:
            ...
            threshold_breached = self.check_threshold(
                value, rule.threshold_operator, threshold
            )
    ...
    async def create_alert(
        self, rule: AlertRule, value: float, threshold: float, metadata: Dict[str, Any]
    ):
        """Create new alert"""
        try:
            ...
            points = [
                {
                    ...
                    "tags": {
                        ...
                        "source_metric": alert.source_metric,
                        "host": (
                            alert.metadata.get("host", "unknown")
                            if alert.metadata
                            else "unknown"
                        ),
                        "service": (
                            alert.metadata.get("service", "unknown")
                            if alert.metadata
                            else "unknown"
                        ),
                    },
        ...
        try:
            ...
            for point in cpu_result.get_points():
                ...
                await self.evaluate_metric(
                    "cpu_usage", cpu_value, {"host": host, "service": "system"}
                )
            ...
            for point in memory_result.get_points():
                ...
                await self.evaluate_metric(
                    "memory_usage", memory_value, {"host": host, "service": "system"}
                )
            ...
            for point in disk_result.get_points():
                ...
                await self.evaluate_metric(
                    "disk_usage",
                    disk_value,
                    {"host": host, "device": device, "service": "system"},
                )
        ...
        try:
            ...
            for point in health_result.get_points():
                ...
                await self.evaluate_metric(
                    "service_status",
                    status,
                    {"service": service, "host": "monitoring_server"},
                )
    ...
    async def collect_application_metrics(self):
        ...
        try:
            ...
            for point in network_result.get_points():
                ...
                await self.evaluate_metric(
                    "network_latency", latency, {"host": host, "service": "network"}
                )
    ...
    async def cleanup_resolved_alerts(self):
        ...
        while self.running:
            try:
                ...
                while (
                    self.alert_history
                    and self.alert_history[0].resolved_at
                    and self.alert_history[0].resolved_at < cutoff_time
                ):
