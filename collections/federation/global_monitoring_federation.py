...
class MetricAggregator:
    ...
    async def aggregate_global_metrics(self, node_metrics: Dict[str, List[Dict]], 
                                     aggregation_rules: Dict[str, str]) -> List[GlobalMetric]:
        ...
        for metric_name, aggregation_method in aggregation_rules.items():
            try:
                ...
                aggregation_func = self.aggregation_methods.get(aggregation_method, self.average_metrics)
    ...
    def sum_metrics(self, metric_values: List[Dict]) -> Tuple[float, float]:
        ...
        confidence = min(1.0, len(metric_values) / 5.0)  # Higher confidence with more data points
    ...
    def percentile_95_metrics(self, metric_values: List[Dict]) -> Tuple[float, float]:
        ...
        confidence = min(1.0, len(values) / 10.0)  # Need more samples for accurate percentiles
        return p95_value, confidence
    
    def weighted_average_metrics(self, metric_values: List[Dict]) -> Tuple[float, float]:
...
class NodeHealthMonitor:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            try:
                                ...
                                health_result['status'] = NodeStatus(data.get('status', 'unknown'))
                            except:
                ...
                if health_result['api_accessible']:
                    metrics_available = await self.check_metrics_availability(session, node, headers)
    ...
    async def check_metrics_availability(self, session: aiohttp.ClientSession, 
                                       node: FederationNode, headers: Dict[str, str]) -> bool:
    ...
    def get_node_health_summary(self, node_id: str) -> Dict[str, Any]:
        ...
        online_checks = sum(1 for h in history if h['status'] in [NodeStatus.ONLINE, NodeStatus.DEGRADED])
...
class DataSynchronizer:
    ...
    async def sync_node_data(self, source_node: FederationNode, 
    ...
    async def fetch_node_data(self, node: FederationNode, data_types: List[str]) -> Dict[str, Any]:
                    ...
                    try:
                        ...
                        async with session.get(url, headers=headers, 
                            ...
                            if response.status == 200:
                                ...
                                logger.warning(f"Failed to fetch {data_type} from {node.node_id}: HTTP {response.status}")
                    
                    except Exception as e:
                        logger.error(f"Error fetching {data_type} from {node.node_id}: {e}")
                        continue
    ...
    async def sync_to_target_node(self, data: Dict[str, Any], target_node: FederationNode,
                                data_types: List[str]) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                for data_type in data_types:
                    ...
                    async with session.post(url, json=data[data_type], headers=headers,
...
class GlobalMonitoringFederation:
    ...
    async def collect_global_metrics(self):
        ...
        while self.running:
            try:
                ...
                if node_metrics:
                    global_metrics = await self.metric_aggregator.aggregate_global_metrics(
                        ...
                        async with session.get(url, headers=headers,
                            ...
                            if response.status == 200:
                                ...
                                if '/api/v1/query' in endpoint:
                                    ...
                                    data = await response.json()
                                    parsed_metrics = self.parse_prometheus_metrics(data, node.node_id)
                                    metrics.extend(parsed_metrics)
                                elif '/metrics' in endpoint:
                                    # Prometheus exposition format
                                    text = await response.text()
                                    parsed_metrics = self.parse_prometheus_exposition(text, node.node_id)
                                    metrics.extend(parsed_metrics)
                    
                    except Exception as e:
                        logger.debug(f"Error collecting from {endpoint} on {node.node_id}: {e}")
                        continue
    ...
    def parse_prometheus_metrics(self, data: Dict[str, Any], node_id: str) -> List[Dict[str, Any]]:
    ...
    def parse_prometheus_exposition(self, text: str, node_id: str) -> List[Dict[str, Any]]:
                    ...
                    if len(parts) == 2:
                        ...
                        try:
                            ...
                            if '{' in metric_part:
                                ...
                                label_part = metric_part[metric_part.find('{'):metric_part.rfind('}')+1]
    ...
    async def monitor_node_health(self):
        ...
        while self.running:
            try:
                ...
                for node_id, node in self.federation_nodes.items():
                    task = asyncio.create_task(self.health_monitor.check_node_health(node))
    ...
    def update_node_statistics(self):
        ...
        online_count = sum(1 for node in self.federation_nodes.values() 
                          if node.status == NodeStatus.ONLINE)
    ...
    async def process_cross_site_alert(self, alert: Dict[str, Any], source_node_id: str):
    ...
    def should_propagate_alert(self, alert: Dict[str, Any], source_node_id: str) -> bool:
    ...
    async def propagate_alert_to_nodes(self, alert: CrossSiteAlert):
        ...
        logger.info(f"Would propagate alert {alert.alert_id} to {len(self.federation_nodes)-1} other nodes")
        ...
        return {
            'federation_nodes': {
                node_id: {
                    ...
                    'health_summary': self.health_monitor.get_node_health_summary(node_id)
                }
                for node_id, node in self.federation_nodes.items()
            },
