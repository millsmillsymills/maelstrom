...
class ResourceOptimizer:
    ...
    def analyze_resource_usage(
        self, host_metrics: Dict, container_metrics: Dict
        ...
        if host_metrics:
            ...
            if cpu_percent >= RESOURCE_THRESHOLDS["cpu_critical"]:
                ...
                recommendations.append(
                    "Monitor CPU usage trends and prepare for scaling"
                )
            ...
            if disk_percent >= RESOURCE_THRESHOLDS["disk_critical"]:
                ...
                recommendations.append(
                    "Schedule disk cleanup and review data retention policies"
                )
        ...
        for name, metrics in container_metrics.items():
            ...
            if health_status in ["unhealthy", "starting"] or metrics.get(
                "status"
            ) not in ["running"]:
        ...
        if high_cpu_containers:
            issues.append(
                f"High CPU containers: {', '.join([f'{name} ({cpu:.1f}%)' for name, cpu in high_cpu_containers])}"
            )
            recommendations.append(
                "Review and optimize high CPU containers or increase resource limits"
            )

        if high_memory_containers:
            issues.append(
                f"High memory containers: {', '.join([f'{name} ({mem:.1f}%)' for name, mem in high_memory_containers])}"
            )
            recommendations.append(
                "Consider restarting high memory containers or increasing memory limits"
            )
    ...
    def apply_optimizations(
        self, recommendations: List[str], host_metrics: Dict, container_metrics: Dict
    ):
        ...
        try:
            ...
            if disk_percent >= RESOURCE_THRESHOLDS["disk_critical"]:
                logger.info(
                    "Applying automatic Docker cleanup due to critical disk usage"
                )
                ...
                optimizations_applied.append(
                    f"Docker cleanup: containers={len(pruned_containers.get('ContainersDeleted', []))}, "
            ...
            for name, metrics in container_metrics.items():
                if (
                    metrics.get("health_status") == "unhealthy"
                    or metrics.get("status") not in ["running"]
                    and metrics.get("status") != "exited"
                ):
            ...
            if unhealthy_containers:
                optimizations_applied.append(
                    f"Restarted containers: {', '.join(unhealthy_containers)}"
                )
            ...
            if memory_percent >= RESOURCE_THRESHOLDS["memory_critical"]:
                ...
                if high_memory_containers:
                    high_memory_containers.sort(
                        key=lambda x: x[1].get("memory_percent", 0), reverse=True
                    )
                    ...
                    if not any(
                        service in container_name.lower()
                        ...
                        try:
                            ...
                            container.restart()
                            optimizations_applied.append(
                                f"Restarted high-memory container: {container_name}"
                            )
                            logger.info(
                                f"Restarted high-memory container: {container_name}"
                            )
                            ...
                            logger.error(
                                f"Failed to restart high-memory container {container_name}: {e}"
                            )
    ...
    def send_alerts(
        self, issues: List[str], recommendations: List[str], host_metrics: Dict
    ):
        ...
        try:
            ...
            message = (
                f"{severity} Resource Alert - {len(alerts_to_send)} issues detected"
            )
                ...
                container_point = {
                    ...
                    "fields": {
                        ...
                        "memory_usage_bytes": int(
                            container_data.get("memory_usage_bytes", 0)
                        ),
                        "memory_percent": float(
                            container_data.get("memory_percent", 0)
                        ),
                        "network_rx_bytes": int(
                            container_data.get("network_rx_bytes", 0)
                        ),
                        "network_tx_bytes": int(
                            container_data.get("network_tx_bytes", 0)
                        ),
                        "block_read_bytes": int(
                            container_data.get("block_read_bytes", 0)
                        ),
                        "block_write_bytes": int(
                            container_data.get("block_write_bytes", 0)
                        ),
...
"""
        ...
        if metrics.containers:
            running_containers = {
                name: data
                for name, data in metrics.containers.items()
            ...
            if running_containers:
                ...
                top_memory = sorted(
                    running_containers.items(),
                    key=lambda x: x[1].get("memory_percent", 0),
                    reverse=True,
                )[:5]
    ...
    def run_monitoring_cycle(self):
        ...
        try:
            ...
            metrics = ResourceMetrics(
                ...
                host_network_bytes_sent=host_metrics.get("network", {}).get(
                    "bytes_sent", 0
                ),
                host_network_bytes_recv=host_metrics.get("network", {}).get(
                    "bytes_recv", 0
                ),
            ...
            # Analyze resources
            issues, recommendations = self.analyze_resource_usage(
            ...
            report = self.generate_resource_report(
                metrics, issues, recommendations, optimizations
            )
            ...
            logger.info(
                f"Monitoring cycle complete: {len(issues)} issues, {len(recommendations)} recommendations, {len(optimizations)} optimizations"
            )
